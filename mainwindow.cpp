...
void MainWindow::InsertFlagItem(const QString & key, const QString & value)
{
    ...
    valueColumn->setFlags(valueColumn->flags()|Qt::ItemIsEditable);
...
void MainWindow::on_actionExport_triggered()
{
    ...
    if (dir.length() == 0)
    {
        QMessageBox::critical(this, "Failed to get dir", "Faild to get directory to save"
                              , QMessageBox::Ok);
        return;
    }
...
void MainWindow::PrepareCMakeListsVariables()
{
    ...
    for (int row = 0; row < ui->sourcesTable->rowCount(); row++)
    {
        ...
        // target, source files
        cmakelists.AddSousrce(target->text(), sources->text());
    }
